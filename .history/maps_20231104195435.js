...
window.onload = function() {
    ...
    // run through the 5 clostest hospitals and query the API for the driving time between the user's location and each hospital using google distance matrix
    // const distanceMatrixUrl = `https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${userLocation}&destinations=${closestHospitalNames}&key=${apiKey2}`;
...
window.onload = function() {
    ...
    const apiKey2 = 'YOUR_API_KEY'; // Replace with your Google Distance Matrix API key
    // Function to convert ZIP code to lat/long
    function convertZipCode() {
        // Get the ZIP code from the form
        const zipCode = document.getElementById('search-input').value;
    ...
    fetch(facilityList)
        ...
        .then(data => {
            ...
            const closestHospitalNames = allHospitals.slice(0, 5).map(hospital => hospital.name).join(', ');
            console.log(`The 5 closest hospitals are: ${closestHospitalNames}.`);
            // run through the 5 closest hospitals and query the API for the driving time between the user's location and each hospital using Google Distance Matrix
            const distanceMatrixUrl = `https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${userLocation.lat},${userLocation.lng}&destinations=${closestHospitalNames}&key=${apiKey2}`;
            fetch(distanceMatrixUrl)
                .then(response => response.json())
                .then(data => {
                    const drivingTimes = data.rows[0].elements.map(element => element.duration.text);
                    console.log(`The driving times to the 5 closest hospitals are: ${drivingTimes.join(', ')}.`);
                });
        });
};