...
// ----------------------------------------------------------------------------
    ...
    // create a list of the 5 closest hospitals
    const closestHospitals = [];
    for (let i = 0; i < Math.min(5, allHospitals.length); i++) {
        closestHospitals.push(allHospitals[i]);
    }
    // display the 5 closest hospitals to an alert
    const closestHospitalNames = closestHospitals.map(hospital => hospital.name).join(', ');
    alert(`The 5 closest hospitals are: ${closestHospitalNames}.`);
    };
