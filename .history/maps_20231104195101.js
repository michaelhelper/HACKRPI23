...
window.onload = function() {
    ...
    // run through the 5 clostest hospitals and query the API for the driving time between the user's location and each hospital
    const closestHospitals = allHospitals.slice(0, 5);
    closestHospitals.forEach(hospital => {
        const hospitalName = hospital.name;
        const hospitalCoords = hospital.coords;
        const hospitalUrl = `https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${userLocation}&destinations=${hospitalCoords}&key=${apiKey}`;
        fetch(hospitalUrl)
            .then(response => response.json())
            .then(data => {
                const distance = data.rows[0].elements[0].distance.text;
                const duration = data.rows[0].elements[0].duration.text;
                console.log(`The distance from ${userLocation} to ${hospitalName} is ${distance} and the driving time is ${duration}.`);
            });
    });
};