window.onload = function() {
    ...
    // For each of the 5 closest hospitals, query the API for the driving time between the user's location and each hospital using Google Distance Matrix API
    // https://developers.google.com/maps/documentation/distance-matrix/start
    // write the driving time to the console
    forEach(closestHospitals, function(hospital) {
        const hospitalName = hospital.name;
        const hospitalDistance = hospital.distance;
        const drivingTimeUrl = `https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${userLocation}&destinations=${hospitalName}&key=${apiKey}`;
        fetch(drivingTimeUrl)
            .then(response => response.json())
            .then(data => {
                const drivingTime = data.rows[0].elements[0].duration.text;
                console.log(`The driving time from your location to ${hospitalName} is ${drivingTime}.`);
            });
    });
}
